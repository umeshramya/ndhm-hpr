...
type HI_TYPES = "DiagnosticReportRecord" | "DischargeSummaryRecord" | "HealthDocumentRecord" | "ImmunizationRecord" | "OPConsultRecord" | "PrescriptionRecord" | "WellnessRecord"
...
export default class Link extends Header {
    ...
    notify = async (config: {
        healthId: string, patientReference: any, careContextReference: any, hiTypes: HI_TYPES[], date: string, hipId: string
    }) => {
        const headers = this.headers(config.healthId)
